...
impl OpenFile {
    ...
    pub fn from_fd(pid: usize, fd: usize) -> Option<OpenFile> {
        ...
        let fd_file_path = format!("/proc/{}/fd/{}", pid, fd);
        let path_buf = fs::read_link(fd_file_path).ok()?;
        let path_name = OpenFile::path_to_name(path_buf.to_str()?);

        let fd_info_path = format!("/proc/{}/fdinfo/{}", pid, fd);
        let fd_info = fs::read_to_string(fd_info_path).ok()?;
        let file_cursor = OpenFile::parse_cursor(&fd_info)?;
        let file_access_mode = OpenFile::parse_access_mode(&fd_info)?;
...
mod test {
    ...
    fn test_openfile_from_fd_invalid_fd() {
        ...
        assert!(
            OpenFile::from_fd(process.pid, 130).is_none(),
            "Expected None because file descriptor 130 is invalid"
        );
        let _ = test_subprocess.kill();
    }
}
